...
type ReportPlugin struct {
	...
	FailedFilter4  []*ReportTestFailure `json:"failedTestsFilter4,omitempty"`
	FailedFilter5  []*ReportTestFailure `json:"failedTestsFilter5,omitempty"`
	FailedFilter6  []*ReportTestFailure `json:"failedTestsFilter6,omitempty"`
	FailedFiltered []*ReportTestFailure `json:"failedFiltered,omitempty"`
}
...
func (re *ReportData) populatePluginConformance(rs *summary.ResultSummary, reResult *ReportResult, pluginID string) error {
	...
	if reResult.Plugins[pluginID].Stat.FilterFailedPrio != 0 {
		pluginAlert = "danger"
		pluginAlertMessage = fmt.Sprintf("%d", int64(len(pluginSum.FailedFilter3)))
