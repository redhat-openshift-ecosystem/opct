...
func (cs *ConsolidatedSummary) applyFilterSuiteForPlugin(pluginName string) error {
	...
	log.Debugf("Filter1 (SuiteOnly) results: plugin=%s out=filter(%d) filterExcluded(%d)",
		pluginName, len(ps.FailedFilter1), len(ps.FailedExcludedFilter1))
	return nil
}
...
func (cs *ConsolidatedSummary) applyFilterBaselineForPlugin(pluginName string, filterID string) error {
	...
	log.Debugf("Filter2 (Baseline) results: plugin=%s in=filter(%d) out=filter(%d) filterExcluded(%d)",
...
func (cs *ConsolidatedSummary) applyFilterBaselineAPIForPlugin(pluginName string) error {
	...
	doneFilter := func() {
		log.Debugf("Filter4 (BaselineAPI) results: plugin=%s in=filter(%d) inApi=(%d) out=filter(%d) excluded(%d)",
...
func (cs *ConsolidatedSummary) applyFilterKnownFailuresForPlugin(pluginName string, filterID string) error {
	...
	log.Debugf("Filter5 (KF) results: plugin=%s in=filter(%d) out=filter(%d) filterExcluded(%d)",
...
func (cs *ConsolidatedSummary) applyFilterReplayForPlugin(pluginName string, filterID string) error {
	...
	log.Debugf("Filter6 (Replay) results: plugin=%s in=filter(%d) replay=pass(%d) fail(%d) out=filter(%d) filterExcluded(%d)",
