...
func processResult(input *Input) error {
	...
	// useless as the json file is saved in the result directory
	// if input.json {
	// 	timers.Add("report-total")
	// 	resReport, err := report.ShowJSON()
	// 	if err != nil {
	// 		return fmt.Errorf("error showing report in json format: %v", err)
	// 	}
	...
	if input.saveTo != "" && !input.serverSkip {
		...
		log.Infof("To get started open the report http://%s/index.html.", input.serverAddress)
