...
func init() {
	...
	cmdPublish.Flags().StringVarP(
		&argsPublish.fileType, "type", "t", fileTypeArchive,
		"Result type. Default: archive. Valid values: archive | summary",
	)
}
...
func publishResult(input *submitInput) error {
	...
	requireMatchPrefix := input.fileType == fileTypeArchive
	filename := filepath.Base(input.archive)
	objectKey := fmt.Sprintf("uploads/%s", filename)
	if input.objectKey != "" {
		if !strings.HasSuffix(input.objectKey, filename) && requireMatchPrefix {
